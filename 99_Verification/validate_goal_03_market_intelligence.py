...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="atlas-goal03-") as tmp:
            ...
            _check("price_volume_live", channels.get("price_volume") == "LIVE", result)
            _check("missing_channels_explicit", all(channels.get(key) == "NOT_CONFIGURED" for key in ["market_breadth", "news_announcement", "narrative_attention", "macro_policy"]), result)
            ...
            _check("state_api_market_live", state_market.get("channels", {}).get("price_volume") == "LIVE", result)
