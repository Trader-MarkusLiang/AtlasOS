...
def render_markets_page(market_state: Mapping[str, Any]) -> str:
    ...
    observation_rows = "\n".join(
        ...
        f"<td>{escape(str(item.get('data_quality_status', item.get('type', ''))))}</td>"
        f"<td>{escape(str(item.get('freshness', '')))}</td>"
        f"<td>{escape(str(item.get('timestamp', '')))}</td>"
        "</tr>"
        for item in observations[:12]
        if isinstance(item, Mapping)
    ) or f'<tr><td colspan="5">{escape(t("home.waiting_signal", lang))}</td></tr>'
...
<section class="card"><h2>{escape(t("markets.latest_observations", lang))}</h2><table><thead><tr><th>{escape(t("markets.asset", lang))}</th><th>{escape(t("markets.source", lang))}</th><th>{escape(t("page.status", lang))}</th><th>{escape(t("markets.freshness", lang))}</th><th>{escape(t("markets.timestamp", lang))}</th></tr></thead><tbody>{observation_rows}</tbody></table></section>
</main></body></html>"""
